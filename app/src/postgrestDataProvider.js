            ...
            switch (typeof filters[key]) {
                ...
                case 'object':
                    Object.keys(filters[key]).map( (val) => {
                          ...
                          if(!rest[key]){
                              //If one condition per field
                              rest[key] = `${val}.${filters[key][val]}`;
                          }else{
                              //If few conditions
                              rest[key] = [rest[key]];
                              rest[key].push(`${val}.${filters[key][val]}`)
                          }
                          return 1;
//                        return rest[`${key}->>${val}`] = `eq.${filters[key][val]}`
